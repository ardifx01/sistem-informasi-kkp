...
export default class StatsService {
  ...
  static async getStatsMap(): Promise<ResponsePayload> {
    ...
    const dataPelabuhan = data.slice(21, 90); // 69 baris
    const groups = Math.floor(dataPelabuhan.length / 3); // 23 pelabuhan
    const limit = Math.min(groups, dataMap.length); // jaga-jaga bila beda

    const dataResult = [];

    for (let g = 0; g < limit; g++) {
      const base = g * 3;
      const stat = dataMap[g] || {};

      dataResult.push({
        name: dataPelabuhan[base]?.[2] ?? "",
        region: stat.region ?? null,
        lat: stat.lat ?? null,
        lng: stat.lng ?? null,
        employees: {
          male: Number(dataPelabuhan[base]?.[15]) || 0,
          female: Number(dataPelabuhan[base + 1]?.[15]) || 0,
        },
      });
    }
